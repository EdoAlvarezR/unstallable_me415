...
function steadylevelsweep(m::motordef, p::propdef, ac::acdef, Vmin, Vmax, Omegamax, imax, Tmax;
                            save_path=nothing,
                            plot_etha=true, plot_trust=true, plot_current=true,
                            plot_omega=true, plot_throttle=true)
    ...
    if plot_throttle
      figure()
      plot(Vvec, throttle, "k")
      xlabel("flight speed (m/s)")
      ylabel("throttle")
      ylim([0, 1])
      grid(true, color="0.8", linestyle="--")
      if save_path!=nothing; savefig(save_path*"Thr_vs_V.eps"); end;
    end
    ...
    if plot_etha
      ...
      plot(Vvec, etam, "--k")
      plot(Vvec, etap, "-.k")
      plot(Vvec, etam.*etap, "-k")
      xlabel("flight speed (m/s)")
      ylabel("efficiency")
      legend(["motor", "prop", "combined"])
      ylim([0, 1])
      grid(true, color="0.8", linestyle="--")
      if save_path!=nothing; savefig(save_path*"eff_vs_V.eps"); end;
    end


    if plot_omega
      figure()
      plot(Vvec, OmegaRPM, "k")
      xlabel("flight speed (m/s)")
      ylabel("Omega (RPM)")
      ylim([0, Omegamax])
      grid(true, color="0.8", linestyle="--")
      if save_path!=nothing; savefig(save_path*"RPM_vs_V.eps"); end;
    end
    ...
    if plot_current
      ...
      ylim([0, imax])
      grid(true, color="0.8", linestyle="--")
      if save_path!=nothing; savefig(save_path*"I_vs_V.eps"); end;
    end
    ...
    if plot_trust
      ...
      ylim([0, Tmax])
      grid(true, color="0.8", linestyle="--")
      if save_path!=nothing; savefig(save_path*"T_vs_V.eps"); end;
    end

    return Vvec, etam.*etap, Tp
end
