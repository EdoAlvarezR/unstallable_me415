...
function nondimrun(AR, tr, Lambda, twist, alpha, clmax)
    ...
    text(-b/2.0, cr/4.0 + 0.4, @sprintf("CDi = %0.5f", CDi[1]))
    text(-b/2.0, cr/4.0 + 0.2, @sprintf("e_inv = %0.3f", e[1]))
...
function dimrun(b, chi, chord, theta, Lambda, alpha, clmax)
    ...
    cr = 2*Sref/(b*(1+chord[3]/chord[1]))

    # figure(figsize=(8, 3))
    subplot(121)
    plotwing(chord, b, chi, Lambda)
    # text(-b/2.0, cr/4.0 + 0.6*20, @sprintf("CL = %0.3f", CL))
    # text(-b/2.0, cr/4.0 + 0.4*20, @sprintf("CDi = %0.5f", CDi[1]))
    # text(-b/2.0, cr/4.0 + 0.2*20, @sprintf("e_inv = %0.3f", e[1]))
    # text(-b/2.0, cr/4.0 + 0.0*20, @sprintf("S_ref = %0.3f", Sref))
    # println("")
    # println("")
    # println("CL = ", CL)
    # println("CDi = ", CDi)
